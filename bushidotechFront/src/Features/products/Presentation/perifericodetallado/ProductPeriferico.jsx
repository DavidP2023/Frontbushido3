...
const ProductPlaca = () => {
  ...
  return product ? (
    <div className="max-w-5xl mx-auto my-10 p-8 bg-white rounded-lg shadow-2xl">
        ...
        <div className="w-full md:w-1/2 md:pl-8">
          ...
          <ul className="mb-6 space-y-2">
            {product.specs.map(spec => (
              <li key={spec.key} className="text-lg text-gray-700">
                <span className="font-semibold">{spec.key}:</span> {spec.value}
              </li>
            ))}
          </ul>
          <div className="mb-6">
            <span className="text-3xl font-bold text-orange-600 mr-4">${product.priceCurrent}</span>
            <span className="text-2xl text-gray-400 line-through">${product.priceBefore}</span>
          </div>
